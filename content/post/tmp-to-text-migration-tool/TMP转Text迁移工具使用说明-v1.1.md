...
`转换选中对象及子物体` 会处理当前选中对象下面的所有 `TextMeshProUGUI`。

`转换当前场景所有 TMP` 会扫描当前打开场景中的所有 `TextMeshProUGUI`，并在转换前弹出确认框。
...
5. 对常用 TMP 字体配置字体映射。
6. 先选中一个小范围 UI 节点，使用 `将 TMP 转换为 Text` 或 `转换选中对象及子物体` 试转。
7. 检查转换后的 UI 显示效果。
8. 确认设置无误后，再使用 `转换当前场景所有 TMP` 进行场景级批量转换。
...
### 为什么没有直接转换整个 Project 的按钮？

整个 Project 级别的转换会涉及 prefab、未打开场景、变体 prefab、嵌套 prefab 和资源保存顺序，风险比当前场景转换高很多。

当前 1.0 版本建议先使用选中对象批量转换和当前场景批量转换。需要处理 prefab 时，可以打开 prefab 或把目标 prefab 实例选中后再转换。
