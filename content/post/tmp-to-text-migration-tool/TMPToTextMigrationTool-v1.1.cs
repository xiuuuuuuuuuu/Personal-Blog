...
namespace ProjectTools.Editor
{
    ...
    internal sealed class TMPToTextMigrationSettingsWindow : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                if (GUILayout.Button("转换选中对象及子物体"))
                    TMPToTextMigrationTool.ConvertSelectionFromWindow();

                if (GUILayout.Button("转换当前场景所有 TMP"))
                    TMPToTextMigrationTool.ConvertCurrentSceneFromWindow();
            }
    ...
    internal static class TMPToTextMigrationTool
    {
        ...
        {
            ConvertTMPs(CollectSelectionTMPs(), "当前选择中没有 TextMeshProUGUI 组件。");
        }

        public static void ConvertSelectionFromWindow()
        {
            ConvertSelection();
        }

        public static void ConvertCurrentSceneFromWindow()
        {
            ConvertCurrentSceneWithConfirmation();
        }

        private static List<TextMeshProUGUI> CollectSelectionTMPs()
        {
        ...
        private static List<TextMeshProUGUI> CollectCurrentSceneTMPs()
        {
            var tmps = new List<TextMeshProUGUI>();
        ...
        private static void ConvertCurrentSceneWithConfirmation()
        {
            List<TextMeshProUGUI> tmps = CollectCurrentSceneTMPs();
            if (tmps.Count == 0)
            {
                EditorUtility.DisplayDialog("没有找到 TMP", "当前场景中没有 TextMeshProUGUI 组件。", "确定");
                return;
            }

            bool confirmed = EditorUtility.DisplayDialog(
                "转换当前场景所有 TMP",
                $"将把当前场景中的 {tmps.Count} 个 TextMeshProUGUI 转换为 Text。此操作支持 Undo，是否继续？",
                "转换",
                "取消");
            ...
            ConvertTMPs(tmps, "当前场景中没有 TextMeshProUGUI 组件。");
        }

        private static void ConvertTMPs(List<TextMeshProUGUI> tmps, string emptyMessage)
        {
            if (tmps == null || tmps.Count == 0)
            {
                EditorUtility.DisplayDialog("没有找到 TMP", emptyMessage, "确定");
                return;
            }
